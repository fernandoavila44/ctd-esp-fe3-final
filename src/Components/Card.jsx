...
const Card = ({ name, username, id, remove }) => {
  ...
  const actionFav = () => {
    ...
    if(!localStorage.getItem("favorites")){
      ...
      alert("El dentista fue agregado a tus favoritos")
    } else{
  ...
  return (
    ...
    <div className="card">
      <Link to={`/dentist/${id}`}>
        ...
        <img src={doctorImage} alt='doc-img' className="imgCard" />
      ...
      <button onClick={actionFav} className="favButton">{pathname === "/home" ? "Add" : "Remove"}</button>
    </div>
  );
};
